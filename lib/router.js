...
FlowRouter.route('/ambassadors',{
  name:'ambassadors',
  action : function(){
    BlazeLayout.render("mainLayout",{content:"ambassadors"});
  }
});
...
FlowRouter.route('/change-the-world',{
  name:'change',
  action : function(){
    BlazeLayout.render("mainLayout",{content:"changeTheWorld"});
  }
});
...
FlowRouter.route('/autologin/:token',{
  name:"autologin",
  action: function(){
    BlazeLayout.render("mainLayout",{content: "autologin"});
  }
});
...
FlowRouter.route('/class/:lessonId/login',{
...
FlowRouter.route('/admin/lessons/update/find', {
  name:"findLessonByAdmin",
  action: function(){
    if(Meteor.user().emails[0].address ==="faycal@trys.be" || Accounts.user().emails[0].address ==="vincent@trys.be"){
      BlazeLayout.render("adminLayout",{content:"findLessonByAdmin"});
    } else {
      FlowRouter.go('/');
    }
  }
...
FlowRouter.route('/admin/lessons/update/:id', {
  name:"updateLessonByAdmin",
  action: function(){
    if(Meteor.user().emails[0].address ==="faycal@trys.be" || Accounts.user().emails[0].address ==="vincent@trys.be"){
      BlazeLayout.render("adminLayout",{content:"updateLessonByAdmin"});
    } else {
      FlowRouter.go('/');
    }
  }
